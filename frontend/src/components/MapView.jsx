...
import { validGPS } from './telemetryView';
...
export default function MapView({ unit }) {
  ...
  const heading = <div className="location-panel__heading"><span>⌖ LOCATION</span><b>{validGPS(gps) ? unit?.online ? "GPS FIX" : "LAST FIX" : "NO FIX"}</b></div>;
  if (!validGPS(gps)) return <div className="location-panel">{heading}<div className="location-panel__body"><div><strong>GPS SIGNAL · NO FIX</strong><small>{Number.isFinite(gps?.satellites) ? `${gps.satellites} satellites` : 'Satellite count not available'}<br />GPS UNAVAILABLE · No fallback position.</small></div></div></div>;
