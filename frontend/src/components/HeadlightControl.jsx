...
import { BACKEND_URL } from '../api/socket';
import './HeadlightControl.css';

export default function HeadlightControl({ unit }) {
  ...
  const endpoint = `${BACKEND_URL}/api/units/${encodeURIComponent(unit?.unitId || '')}/headlight`;
  useEffect(() => {
  ...
  async function command(state) {
    if (!enabled || !['ON', 'OFF'].includes(result.state) || inFlight.current) return;
    inFlight.current = true;
    commandVersion.current++;
    setBusy(true);
    const current = generation.current;
    try {
      const response = await fetch(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ state }), signal: AbortSignal.timeout(6000) });
      const data = await response.json();
  ...
  return <section className={`headlight headlight--${state.toLowerCase()}`} aria-label="Headlight control">
    ...
    <button aria-label="Headlight on" aria-pressed={state === 'ON'} disabled={!enabled || busy || !['ON', 'OFF'].includes(state)} onClick={() => command('ON')}>ON</button>
    <button aria-label="Headlight off" aria-pressed={state === 'OFF'} disabled={!enabled || busy || !['ON', 'OFF'].includes(state)} onClick={() => command('OFF')}>OFF</button>
